...
import { Connect, WalletInstance, CoinbaseInstance } from "./hooks/useConnectWallet";
import About from "./Views/Other/About";
import PrivacyPolicy from "./Views/Other/PrivacyPolicy";
...
function App() {
  ...
  useEffect(() => {
    ...
    // Check for Coinbase connection
    try {
      // Assuming CoinbaseInstance handles the event similarly to MetaMask
      CoinbaseInstance.on("accountsChanged", () => setReload(reload + 1));
    } catch(error) {
      console.log("Coinbase is not found");
    }

    // Check for existing Coinbase connection
    if (localStorage.getItem("IsCoinbaseConnect")) {
      Connect('Coinbase');
    }
  }, [reload]);
