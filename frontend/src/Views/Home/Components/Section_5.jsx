  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-10 flex-wrap">
        {Creator ? (
          Creator.length > 0 ? (
            <div className="flex w-full justify-center sm:justify-start gap-4 flex-wrap">
              {Creator.map((item, index) => (
                <Link
                  to={`/author/${item.userEthAddress}`}
                  key={index}
                  className="flex flex-autoe  w-[14pc] h-[14pc] cursor-pointer items-center relative justify-start flex-col gap-2"
                >
                  <img
                    src={item.userProfile ? item.userProfile : demoUserAvatar}
                    className="h-[10pc] border-[1px] border-gray-700/50 rounded-lg w-[10pc]"
                    alt=""
                  />
